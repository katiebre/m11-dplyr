...
# Read in the data
pulitzer <- read.csv("~/desktop/info/m11-dplyr/m11-dplyr/exercise-8/data/pulitzer-circulation-data.csv", stringsAsFactors = FALSE)
...
print(str(pulitzer))
#I was surprised that some of the words are stored as characters instead of strings. I think they are characters because characters are easier to sort through as opposed to entire strings.
...
#Question: Which newspaper won the least Pulitzers from 2004-2014?
least.amount.of.wins<- filter(pulitzer, min(Pulitzer.Prize.Winners.and.Finalists..2004.2014) == 
                                Pulitzer.Prize.Winners.and.Finalists..2004.2014) %>% 
  select(Newspaper)
